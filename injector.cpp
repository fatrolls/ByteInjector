...
int CInjector::InjectAuto(std::wstring dllPath, std::wstring processPath)
{
	...
	try {
		...
		LPCWSTR lpApplicationName = processPath.c_str(); /* The program to be executed */

		/* Create the process */
		if (!CreateProcessW(lpApplicationName, NULL, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, exeDirectory.c_str(), &si, &pi)) {
			char errorMsg[1000];
			sprintf_s(errorMsg, 1000, "Uh-Oh! CreateProcess() failed to start program %ls\n", lpApplicationName);
			MessageBoxA(0, errorMsg, errorMsg, 0);
			return 0;
		}
		else {
